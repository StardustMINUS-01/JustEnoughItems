...
public final class JeiRecipeChainPatternEncodingService {
	...
	private static List<PreparedEntry> prepareEntries(
		...
		for (JeiPatternEncodeRequestWire request : requests) {
			...
			ItemStack encodedPattern = createEncodedPattern(player.level(), request);
	...
	@Nullable
	static ItemStack createEncodedPattern(Level level, JeiPatternEncodeRequestWire request) {
		try {
	...
	static ItemStack createProcessingPattern(JeiPatternEncodeRequestWire request) {
		...
		if (outputs.isEmpty() || !isValidStack(outputs.getFirst())) {
			return null;
		}
	...
	@Nullable
	static ItemStack createCraftingPattern(Level level, JeiPatternEncodeRequestWire request) {
		RecipeHolder<CraftingRecipe> recipeHolder = getCanonicalRecipe(level, request, RecipeType.CRAFTING, CraftingRecipe.class);
		if (recipeHolder == null) {
			return null;
		}
		ItemStack[] ingredients = createCraftingGuideItems(request, recipeHolder);
		if (ingredients == null) {
			return null;
		}
		CraftingInput craftingInput = CraftingInput.of(3, 3, List.of(ingredients));
		if (!recipeHolder.value().matches(craftingInput, level)) {
			return null;
		}
		ItemStack output = recipeHolder.value().assemble(craftingInput, level.registryAccess());
		if (output.isEmpty()) {
			return null;
		}
	...
	@Nullable
	static ItemStack[] createCraftingGuideItems(JeiPatternEncodeRequestWire request, RecipeHolder<CraftingRecipe> recipeHolder) {
	...
	static ItemStack createStonecuttingPattern(Level level, JeiPatternEncodeRequestWire request) {
		RecipeHolder<StonecutterRecipe> recipeHolder = getCanonicalRecipe(level, request, RecipeType.STONECUTTING, StonecutterRecipe.class);
		if (recipeHolder == null) {
			return null;
		}
		...
		ItemStack output = recipeHolder.value().getResultItem(level.registryAccess());
		if (output.isEmpty()) {
			return null;
		}
	...
	static ItemStack createSmithingTablePattern(Level level, JeiPatternEncodeRequestWire request) {
		RecipeHolder<SmithingRecipe> recipeHolder = getCanonicalRecipe(level, request, RecipeType.SMITHING, SmithingRecipe.class);
		if (recipeHolder == null) {
			return null;
		}
		List<Ingredient> expectedIngredients = CraftingRecipeUtil.getIngredients(recipeHolder.value());
		...
		ItemStack output = recipeHolder.value().assemble(recipeInput, level.registryAccess());
		if (output.isEmpty()) {
			return null;
		}
	...
	private static <T extends Recipe<?>> RecipeHolder<T> getCanonicalRecipe(
		...
		RecipeHolder<?> recipeHolder = level.getRecipeManager().byKey(request.canonicalRecipeId()).orElse(null);
		if (recipeHolder == null || recipeHolder.value().getType() != recipeType || !recipeClass.isInstance(recipeHolder.value())) {
			return null;
		}
