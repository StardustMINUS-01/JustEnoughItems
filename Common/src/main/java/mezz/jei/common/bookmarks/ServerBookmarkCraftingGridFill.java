 ...
 * ExternalIngredientSource support ported from JEI 1.21.1 (StardustMINUS-01/JustEnoughItems, branch 1.21.1)
 * and adapted to the 1.20.1 Forge API (NBT stack comparison instead of data components).
 */
	...
	public static int fill(
		...
		if (menu.containerId != containerId || targetStacks.isEmpty()) {
			if (DebugConfig.isDebugModeEnabled()) {
				LOGGER.info("[Bug6] FILL-REJECT-CONTAINER menuContainerId={} packetContainerId={} targetStacks={}",
		...
		if (craftingSlots.isEmpty()) {
			if (DebugConfig.isDebugModeEnabled()) {
				LOGGER.info("[Bug6] FILL-NO-SLOTS menu={}", menu.getClass().getSimpleName());
			}
			return 0;
		}
		...
		if (recipeStacks.stream().allMatch(ItemStack::isEmpty)) {
			if (DebugConfig.isDebugModeEnabled()) {
				LOGGER.info("[Bug6] FILL-ALL-EMPTY targetStacks={}", targetStacks);
			}
			return 0;
		}
		...
		if (!canStowCraftingGrid(craftingSlots, playerSlots)) {
			if (DebugConfig.isDebugModeEnabled()) {
				LOGGER.info("[Bug6] FILL-CANNOT-STOW menu={}", menu.getClass().getSimpleName());
			}
			return 0;
		}
		...
		if (craftCount <= 0) {
			if (DebugConfig.isDebugModeEnabled()) {
				LOGGER.info("[Bug6] FILL-CRAFT-COUNT-0 recipeStacks={} multiplier={}", recipeStacks, multiplier);
			}
			return 0;
		}
		...
		if (filled <= 0) {
			if (DebugConfig.isDebugModeEnabled()) {
				LOGGER.info("[Bug6] FILL-FILLED-0 craftCount={} recipeStacks={}", craftCount, recipeStacks);
			}
			return 0;
		}
		if (DebugConfig.isDebugModeEnabled()) {
			LOGGER.info("[Bug6] FILL-OK craftCount={} filled={} recipeStacks={}", craftCount, filled, recipeStacks);
		}
		...
		for (int i = 0; i < recipeStacks.size(); i++) {
			...
			if (moved.getCount() < amount && externalSource != null) {
				...
				if (!external.isEmpty()) {
					if (moved.isEmpty()) {
						moved = external;
					} else if (CraftingStackMatcher.matchesExactStack(moved, external)) {
