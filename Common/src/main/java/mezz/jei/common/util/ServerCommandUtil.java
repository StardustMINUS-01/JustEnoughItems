...
public final class ServerCommandUtil {
	...
	private static void giveToInventory(Player entityplayermp, ItemStack itemStack) {
		...
		ItemStack template = itemStack.copyWithCount(1);
		...
		for (int slot = 0; slot < inventory.items.size() && remaining > 0; slot++) {
			...
			if (current.isEmpty()) {
				int amount = Math.min(remaining, inventory.getMaxStackSize(template));
				ItemStack added = template.copyWithCount(amount);
				added.setPopTime(5);
				inventory.setItem(slot, added);
				remaining -= amount;
			} else if (current.isStackable() && canStack(current, template)) {
				int amount = Math.min(remaining, inventory.getMaxStackSize(current) - current.getCount());
				if (amount > 0) {
					current.grow(amount);
		...
		if (given > 0) {
			...
			notifyGive(entityplayermp, template.copyWithCount(given));
		}
	}
	...
	private static void notifyGive(Player player, ItemStack stack) {
		...
		// Chat hover item counts are limited to 99; the message carries the actual give count separately.
		Component stackTextComponent = stack.copyWithCount(1).getDisplayName();
