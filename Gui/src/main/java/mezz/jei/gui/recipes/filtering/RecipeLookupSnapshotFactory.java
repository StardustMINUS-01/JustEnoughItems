...
public final class RecipeLookupSnapshotFactory {
	...
	public RecipeLookupSnapshotFactory(IRecipeManager recipeManager, IIngredientManager ingredientManager, IFocusFactory focusFactory, ISearchStorageBuilderFactory searchStorageBuilderFactory) {
	...
	private <T> RecipeLookupSnapshot.RecipeEntry<T> createEntry(
		...
		IRecipeSlotsView ingredients = recipeManager.createRecipeLayoutDrawable(category, recipe, focusFactory.getEmptyFocusGroup())
			.map(layout -> layout.getRecipeSlotsView())
			.orElseGet(() -> () -> List.of());
		List<ITypedIngredient<?>> inputs = ingredients.getSlotViews(RecipeIngredientRole.INPUT).stream()
			.flatMap(IRecipeSlotView::getAllIngredients).toList();
		List<ITypedIngredient<?>> outputs = ingredients.getSlotViews(RecipeIngredientRole.OUTPUT).stream()
			.flatMap(IRecipeSlotView::getAllIngredients).toList();
