...
public final class FavoriteRecipeJsonSerializer {
	...
	private static final Codec<String> FAVORITE_TYPE_CODEC = Codec.STRING.flatXmap(
		type -> TYPE_FAVORITE.equals(type) ? DataResult.success(type) : DataResult.error(() -> "Unknown favorite recipe type: " + type),
		DataResult::success
	);
	...
	public static Codec<FavoriteRecipeStore.Entry> create(
		...
		return RecordCodecBuilder.create(instance -> instance.group(
			...
			.apply(instance, (type, recipeType, recipe, target, inputs) -> new FavoriteRecipeStore.Entry(target, new FocusedRecipe(recipeType, recipe), inputs)
			));
	}
}
