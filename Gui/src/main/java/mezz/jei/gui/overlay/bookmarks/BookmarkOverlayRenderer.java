 ...
 * Ported from JEI 1.21.1 (BookmarkOverlayRenderer.java), adapted to the 1.20.1 PanelSnapshot shape
 * (no BoundaryConnections record) and the 1.20.1 BookmarkHotkeyTooltipUtil.addGroupHotkeys signature.
 */
public class BookmarkOverlayRenderer {
	...
	void drawFavoriteRecipeRowPanels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
	...
	void drawGroupPanelPlaceholderLine(GuiGraphics guiGraphics, ImmutableRect2i slotArea) {
		drawGroupPanelPlaceholderLine(guiGraphics, slotArea, false, false);
	}

	void drawGroupPanelPlaceholderLine(
		GuiGraphics guiGraphics,
		ImmutableRect2i slotArea,
		boolean connectedToPrevious,
		boolean connectedToNext
	...
	boolean drawGroupHotkeyTooltip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		BookmarkHotkeyTooltipUtil.addGroupHotkeys(tooltip, overlay.getKeyBindings(), Screen.hasAltDown(), grouped, craftingMode, canEncodeAe2Patterns());
		tooltip.draw(guiGraphics, mouseX, mouseY);
		return true;
	}
