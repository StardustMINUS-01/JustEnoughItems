...
public final class BookmarkChainSlotVisuals {
	...
	private static Optional<BookmarkSlotVisuals> createVisuals(
		...
		if ((metadata.type().isGraphInput() && displayMode != BookmarkSlotDisplayMode.DEFAULT || metadata.type().isNonConsumable()) && metadata.recipeUid() != null) {
			...
			return Optional.of(new BookmarkSlotVisuals(
				displayMode == BookmarkSlotDisplayMode.DEFAULT ? OptionalInt.empty() : OptionalInt.of(metadata.type().isNonConsumable() ? CATALYST_COLOR : INGREDIENT_COLOR),
				OptionalInt.empty(),
	...
	private static Optional<String> getMultiplierText(BookmarkDisplayEntry<?> entry, RecipeChainItem item, BookmarkSlotDisplayMode displayMode) {
		// 1.21.1 parity: item-only bookmarks have no recipe to be a multiplier of.
