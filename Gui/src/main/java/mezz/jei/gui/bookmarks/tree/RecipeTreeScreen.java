...
public final class RecipeTreeScreen extends Screen {
	...
	private void fit() {
		...
		zoom = RecipeTreeMath.clamp(Math.min((viewportRight() - viewportLeft() - 24) / Math.max(1, tree.width()), (height - TOP - 24) / Math.max(1, tree.height())), 0.0001, 1.5);
		panX = viewportLeft() + (viewportRight() - viewportLeft() - tree.width() * zoom) / 2;
		panY = TOP + 12;
	}

	private void zoomAt(double factor, double x, double y) {
		double next = RecipeTreeMath.clamp(zoom * factor, 0.0001, 3);
	...
	private void drawNode(GuiGraphics graphics, RecipeTreeLayout.Node node) {
		...
		drawOutline(graphics, x, y, RecipeTreeLayout.WIDTH, node.height(), node == selected ? 0xFFE4C86C : node.recipe() ? 0xFF719786 : 0xFF777C7A);
	...
	private void drawSidebar(GuiGraphics graphics, int mouseX, int mouseY) {
		sidebarScroll = RecipeTreeMath.clamp(sidebarScroll, 0, sidebarMaxScroll());
		...
		for (var cell : sidebarLayout().cells()) {
			...
			if (entry.stack() == null) {
				...
				if (!showSummary && selected != null && entry.stack() == stacks.get(selected.target().index())) {
					drawOutline(graphics, x, y, 20, 20, 0xFFE4C86C);
				}
	...
	public boolean mouseScrolled(double x, double y, double vertical) {
		if (candidateInputs.handleMouseScrolled(x, y, vertical)) {
			return true;
		}
		if (y < TOP) {
			return false;
		...
		if (x >= viewportRight() && sidebarOpen()) {
			sidebarScroll = RecipeTreeMath.clamp(sidebarScroll - (int) (vertical * 24), 0, sidebarMaxScroll());
