...
class RecipeTreeSummaryTest {
	@BeforeAll static void bootstrap() { SharedConstants.tryDetectVersion(); Bootstrap.bootStrap(); }

	@Test void availabilityAndMissingUpdateWithoutChangingSavedDemandOrCountingCatalysts() {
	...
	@Test void stockOfFinalProductDoesNotExpandTheOutputTarget() {
	...
	@Test void surplusOfIntermediateRecipeUsesExistingChainMath() {
	...
	@Test void statusSectionsRemainPresentWhenEmpty() {
	...
	private static long amount(RecipeChainTooltipModel model, RecipeChainTooltipSectionType type) {
		return model.sections().stream().filter(section -> section.type() == type).flatMap(section -> section.items().stream()).mapToLong(RecipeChainTooltipModel.Item::amount).sum();
	}
